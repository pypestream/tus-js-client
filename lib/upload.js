...
const {localStorage, btoa} = window;
...
class Upload {
  ...
  start() {
    ...
    this._query = this.options.params
      ? this._constructQuery(this.options.params)
      : this._extractQuery(this.options.endpoint);
  ...
  _extractQuery(url) {
    ...
    return url.indexOf(delimiter) > -1
      ? url.split(delimiter)[1]
      : null;
  }

  _constructQuery(params) {
  ...
  _createUpload() {
    ...
    xhr.onload = () => {
      ...
      this.url = xhr.getResponseHeader("Location") + (this._query ? '?' + this._query : '');
  ...
  _resumeUpload() {
    ...
    xhr.onload = () => {
      ...
      // data to the server
      if (offset === length) {
        const result = xhr.getResponseHeader('File-URL');
        this._emitProgress(length, length);
        this._emitSuccess(result);
        return;
      }
  ...
  _startUpload() {
    ...
    xhr.onload = () => {
      ...
      this._emitChunkComplete(offset - this._offset, offset, this.file.size);
